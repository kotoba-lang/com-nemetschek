(ns nemetschek.main
  "Kotodama WASM entrypoint for the nemetschek clean-room actor (L4) — Clojure port.
  ...
  facts (`nemetschek.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "nemetschek")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"nemetschek.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "nemetschek-compat" :tier tier :entities entities} 200])
...
(defn start! [] :nemetschek-compat/ready)
